...
public class FarmingTeleportOverlay extends Overlay {
    ...
    /**
     * Returns every patch type included in the currently active custom run.
     *
     * This is used by the seed highlighting overlay so it can determine which
     * seed types are actually relevant to the current run.
     */
    public List<String> getActiveRunPatchTypes() {
        List<String> patchTypes = new ArrayList<>();

        if (!customRunMode || customRunLocations == null || customRunLocations.isEmpty()) {
            return patchTypes;
        }

        for (RunLocation location : customRunLocations) {
            if (location == null || location.getPatchTypes() == null) {
                continue;
            }

            patchTypes.addAll(location.getPatchTypes());
        }
    ...
    /**
     * Returns the current Location (with teleport override set) and patch type
     * when in custom run mode; otherwise null.
     */
    private Location getCurrentLocationForCustomRun() {
        if (!customRunMode
                || customRunLocations.isEmpty()
                || currentLocationIndex >= customRunLocations.size()) {
            return null;
        }
        ...
        if (patchTypes == null
                || patchTypes.isEmpty()
                || currentPatchTypeIndex >= patchTypes.size()) {
            return null;
        }
        ...
        Location loc = catalog.getLocationForPatch(
                rl.getLocationName(),
                patchType);
    ...
    private void updateHintArrow(Location location) {
        ...
        // Morytania: slightly larger radius so navigation hint clears before
        // farming steps (aligns with FarmingStepHandler PATCH_HINT_ARROW_RANGE_TILES).
        int clearDistance =
                "Morytania".equals(location.getName()) ? 15 : 5;

        // Special handling for Brimhaven
        boolean isBrimhavenFruitTree =
                customRunMode
                        && PatchTypes.FRUIT_TREE.equals(getCurrentPatchTypeForCustomRun())
                        && "Brimhaven".equals(location.getName());

        if (isBrimhavenFruitTree) {
            if (currentRegionId == 10547) {
                boolean nearBrimhavenPatch =
                        areaCheck.isPlayerWithinArea(patchPoint, 20);

                if (!nearBrimhavenPatch) {
                    WorldPoint captainBarnabyLocation =
                            new WorldPoint(2675, 3265, 0);
        ...
        boolean isEntranaHops =
                customRunMode
                        && PatchTypes.HOPS.equals(getCurrentPatchTypeForCustomRun())
                        && "Entrana".equals(location.getName());

        if (isEntranaHops) {
            boolean nearEntranaPatch =
                    areaCheck.isPlayerWithinArea(patchPoint, 20);

            if (!nearEntranaPatch) {
                WorldPoint entranaMonkLocation =
                        new WorldPoint(3042, 3235, 0);
        ...
        boolean nearPatch =
                areaCheck.isPlayerWithinArea(patchPoint, clearDistance);
    ...
    private void navigateToCurrentLocation(Graphics2D graphics) {
        ...
        navigationHandler.gettingToLocation(
                graphics,
                location,
                getCurrentPatchTypeForCustomRun());
        ...
        if (navigationHandler.isAtDestination) {
            ...
            // Navigation hint points at the patch tile until "near" clears it;
            // destination can be reached while still outside that radius.
    ...
    private void moveToNextPatchOrLocation() {
        ...
        RunLocation rl =
                customRunLocations.get(currentLocationIndex);

        List<String> patchTypes = rl.getPatchTypes();

        if (patchTypes != null
                && currentPatchTypeIndex + 1 < patchTypes.size()) {
    ...
    public void skipCurrentStep() {
        ...
        farmingStepHandler.forceAdvancePatchType(
                getCurrentPatchTypeForCustomRun());
    }
    ...
    public void removeOverlay() {
        ...
        // Clear the selected custom run so seed highlights disappear.
